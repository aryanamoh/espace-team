...
// Constants for level parameters
#define LEVEL_COUNT 4
const int levelProgress[LEVEL_COUNT] = {25, 50, 75, 100};
const int levelTimeLimits[LEVEL_COUNT] = {25, 20, 15, 10}; // Time limits in seconds for each level
...
// Function to display a level reached message
void displayLevelReached(int level) {
  tft.fillScreen(TFT_BLACK);
  tft.setTextSize(2);
  tft.setTextColor(TFT_WHITE);
  tft.setCursor(0, 50);
  tft.println("Level " + String(level + 1) + " reached!");
...
void loop()
{
  ...
  for (int i = 0; i < LEVEL_COUNT; i++) {
    if (progress >= levelProgress[i] && progress > lastProgress) {
      currentLevel = i;
      lastProgress = progress;
      // Display level reached message
      displayLevelReached(currentLevel);
      break;
    }
  }

  // Adjust timers based on current level
  int timeLimit = levelTimeLimits[currentLevel];
  timerAlarmWrite(askExpireTimer, timeLimit * 1000000, true);
